...
dta1 <- dta %>% select(trust_neighbors, exports, age, age2, male,
...
# Table 1: Trust Relatives (Column 1)
m1_vec <- lm(trust_neighbors ~ exports + age + age2 + male + urban_dum 
             + as.factor(education) + as.factor(occupation) +
               as.factor(religion) + as.factor(living_conditions) +
               district_ethnic_frac + frac_ethnicity_in_district +
               as.factor(isocode), 
             data = dta1)
  
# Errores estándar usando Arai (2015)
ee_etnia <- clx(m1_vec, 1, dta1$murdock_name)

ee_etnia_distrito <- mclx(m1_vec, 1, dta1$murdock_name, dta1$district)
...
```{r}
stargazer(m1_vec, type = "text",
          se = list(ee_etnia[,2], ee_etnia_distrito[,2]),
          digits = 5)
```
